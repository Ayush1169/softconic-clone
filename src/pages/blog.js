...
function BlogPage() {
  ...
  const fetchBlogs = (page) => {
    axios.get(`https://blog.evdtechnology.com/wp-json/wp/v2/posts?_embed&per_page=15&page=${page}`)
      .then(response => {
        ...
        <div className="container">
          <div className="row g-lg-4 gy-5">
            {blogs.map((blog) => {
              ...
              const category = blog.categories[0] === 1 ? "Web Development" : "Cloud Solutions";
              const img = _embedded['wp:featuredmedia']?.[0]?.source_url || '/default-image.jpg';
              const count_comment = _embedded.replies?.length || 0;

              return (
                <div key={id} className="col-lg-4 col-md-6 wow animate fadeInLeft" data-wow-delay="300ms" data-wow-duration="1500ms">
                  <div className="single-blog magnetic-item">
                    <div className="blog-img">
                      <img className="img-fluid" src={img} alt={title.rendered} />
                    ...
                    <div className="blog-content">
                      <ul className="blog-meta">
                        <li><Link href="/blog">{new Date(date).toLocaleDateString()}</Link></li>
                        <li><Link href="/blog">Comment ({count_comment})</Link></li>
                      </ul>
